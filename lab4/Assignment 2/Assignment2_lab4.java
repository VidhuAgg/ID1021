...
public class Assignment2_lab4 {
    public static void main(String[] args) throws Exception {
        ...
        BreadthFirstPaths bfs = new BreadthFirstPaths(G, st.get(start)); // get int from ST for start and instantiate
                                                                         // bfs object
        ...
        if (!bfs.hasPathTo(st.get(end))) { // exception if path doesnt exist
            sc.close();
            throw new Exception(" No path found");
        }
        Iterable<Integer> path = bfs.pathTo(st.get(end)); // get path from int value for end from ST and store it
